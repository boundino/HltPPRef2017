...
HLTDisplacedtktkVtxProducer::HLTDisplacedtktkVtxProducer(const edm::ParameterSet& iConfig):	
  ...
  saveobj_ (iConfig.getParameter<int>("saveobj")),  
...

{
  ...
  if(saveobj_>0)
    {
      root = fs->make<TTree>(treename_.c_str(),"");
      root->Branch("hltBSx", &BSx);
      root->Branch("hltBSy", &BSy);
      root->Branch("hltBSz", &BSz);
      root->Branch("tktkDsize", &tktkDsize_);
      root->Branch("tktkDpt", &vtktkDpt_);
      root->Branch("tktkDeta", &vtktkDeta_);
      root->Branch("tktkDphi", &vtktkDphi_);
      root->Branch("tktkDmass", &vtktkDmass_);
      root->Branch("Tksize", &Tksize_);
      root->Branch("Tkpt", &vTkpt_);
      root->Branch("Tketa", &vTketa_);
      root->Branch("Tkphi", &vTkphi_);
    }
}
...
HLTDisplacedtktkVtxProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<int>("saveobj",0);
  desc.add<std::string>("treename","saveobj");
...
void HLTDisplacedtktkVtxProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(saveobj_>0)
    {
      for (cand1=trackcands->begin(); cand1!=trackcands->end(); cand1++) {
        vTkpt_.push_back(cand1->pt());
        vTketa_.push_back(cand1->eta());
        vTkphi_.push_back(cand1->phi());
        Tksize_++;
      }
    }	
  ...
  for (cand1=trackcands->begin(); cand1!=trackcands->end(); cand1++) {
    ...
    for (; cand2!=trackcands->end(); cand2++) {
      ...
      if(saveobj_>0)
        {
          vtktkDpt_.push_back(p.pt());
          vtktkDeta_.push_back(p.eta());
          vtktkDphi_.push_back(p.phi());
          vtktkDmass_.push_back(p.mass());
          tktkDsize_++;
        } 
